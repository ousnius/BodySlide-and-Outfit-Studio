...
wxSubSliderPanel::wxSubSliderPanel()
	: wxWindow() {}

wxSubSliderPanel::wxSubSliderPanel(wxWindow* parent, const wxString& name, const wxBitmap& bmpEdit, const wxBitmap& bmpSettings)
	: wxWindow() {
	Create(parent, name, bmpEdit, bmpSettings);
}

bool wxSubSliderPanel::Create(wxWindow* parent, const wxString& name, const wxBitmap& bmpEdit, const wxBitmap& bmpSettings) {
	if (isCreated) {
...
wxSubSliderPanel* wxSubSliderPanelPool::Push() {
	if (pool.size() < MaxPoolSize) {
		auto entry = new wxSubSliderPanel();
...
void wxSubSliderPanelPool::CreatePool(size_t poolSize, wxWindow* parent, const wxBitmap& bmpEdit, const wxBitmap& bmpSettings) {
...
wxSubSliderPanel* wxSubSliderPanelPool::Get(size_t index) {
	if (pool.size() > index)
		return pool[index];
...
wxSubSliderPanel* wxSubSliderPanelPool::GetNext() {
	for (size_t i = 0; i < pool.size(); ++i) {
		wxSubSliderPanel* sliderPanel = pool[i];
...
void wxSubSliderPanelPool::Clear() {
	for (auto& p : pool)
		p->Destroy();
