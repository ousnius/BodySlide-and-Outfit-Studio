	...
	if (XRCCTRL(wiz, "npRefIsTemplate", wxRadioButton)->GetValue() == true) {
		...
		if (tmpl != refTemplates.end()) {
			if (wxFileName(wxString::FromUTF8(tmpl->GetSource())).IsRelative())
				error = project->LoadReferenceTemplate(Config["AppDir"] + PathSepStr + tmpl->GetSource(), tmpl->GetSetName(), tmpl->GetShape(), tmpl->GetLoadAll());
			else
...
void OutfitStudioFrame::OnLoadReference(wxCommandEvent& WXUNUSED(event)) {
	...
	if (XRCCTRL(dlg, "npRefIsTemplate", wxRadioButton)->GetValue() == true) {
		...
		if (tmpl != refTemplates.end()) {
			if (wxFileName(wxString::FromUTF8(tmpl->GetSource())).IsRelative())
				error = project->LoadReferenceTemplate(Config["AppDir"] + PathSepStr + tmpl->GetSource(), tmpl->GetSetName(), tmpl->GetShape(), tmpl->GetLoadAll(), mergeSliders);
			else
...
void OutfitStudioFrame::CalcCopySkinTransOption(WeightCopyOptions &options) {
	...
	// from base shape" checkbox should be shown and what its default value
	// should be.
	...
	// As a first step in calculating a good default for the option, find
	// the average vertex position of the base shape in its own skin
	// coordinates.
	Vector3 baseAvg;
	{
		...
		for (int i = 0; i < selectedItems.size(); i++) {
			...
			if (!project->IsBaseShape(shape)) {
				...
				if (options.doSkinTransCopy)
					workAnim.ChangeGlobalToSkinTransform(shape->GetName(), workAnim.shapeSkinning[project->GetBaseShape()->GetName()].xformGlobalToSkin);
