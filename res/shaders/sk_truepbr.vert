...
// The vertex stage of the Community Shaders "True PBR" pair, see sk_truepbr.frag. It is the
// default Skyrim one: PBR changes what the fragment stage makes of the maps, not what the geometry
// hands it. Kept as its own file so the pair can be read and changed on its own.
...
void main(void)
{
	...
	// Always built, unlike the default shader which skips it for model space normals. True PBR has no
	// model space mode - its normal map is tangent space by definition - and leaving the matrix out
	// under some condition the fragment stage doesn't share would hand it an undefined varying.
