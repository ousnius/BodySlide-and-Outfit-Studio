		...
		const int loading_as_cubemap)
{
	...
	/*	I need all of these	*/
	const unsigned int dds_flags = DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH | DDSD_PIXELFORMAT;
	...
	/*	I need one of these	*/
	const unsigned int pixel_format_flags = DDPF_FOURCC | DDPF_RGB | DDPF_LUMINANCE;
		...
		for(unsigned int cf_target = ogl_target_start; cf_target <= ogl_target_end; ++cf_target )
		{
			         ...
			         ( header.sPixelFormat.dwRBitMask == 0xf800 || header.sPixelFormat.dwRBitMask == 0x7c00 ) )
			{
				...
				for( int i = 0; i < (int)DDS_full_size; i += block_size )
				{
					...
					unsigned short r = (unsigned short)( ( pixel & header.sPixelFormat.dwRBitMask ) >> 10 );
					unsigned short g = (unsigned short)( ( pixel & header.sPixelFormat.dwGBitMask ) >> 5 );
					unsigned short b = (unsigned short)( ( pixel & header.sPixelFormat.dwBBitMask ) >> 0 );
					unsigned short a = 1;
					if( header.sPixelFormat.dwAlphaBitMask != 0 )
					{ a = (unsigned short)( ( pixel & header.sPixelFormat.dwAlphaBitMask ) >> 15 ); }
			         ...
			         ( header.sPixelFormat.dwAlphaBitMask == 0xf000 ) )
			{
				...
				{
					unsigned short pixel = DDS_data[i] << 0 | DDS_data[i + 1] << 8;
					unsigned short r = (unsigned short)( ( pixel & header.sPixelFormat.dwRBitMask ) >> 8 );
					unsigned short g = (unsigned short)( ( pixel & header.sPixelFormat.dwGBitMask ) >> 4 );
					unsigned short b = (unsigned short)( ( pixel & header.sPixelFormat.dwBBitMask ) >> 0 );
					unsigned short a = (unsigned short)( ( pixel & header.sPixelFormat.dwAlphaBitMask ) >> 12 );
