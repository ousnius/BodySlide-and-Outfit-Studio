namespace gli
{
	...
	inline storage_linear::size_type storage_linear::image_offset(extent1d const& Coord, extent1d const& ImageExtent) const
	{
		GLI_ASSERT(glm::all(glm::lessThan(Coord, ImageExtent)));
		static_cast<void>(ImageExtent);
		return static_cast<size_t>(Coord.x);
	}

	inline storage_linear::size_type storage_linear::image_offset(extent2d const& Coord, extent2d const& ImageExtent) const
	{
		GLI_ASSERT(glm::all(glm::lessThan(Coord, ImageExtent)));
		return static_cast<size_t>(Coord.x + Coord.y * ImageExtent.x);
	}

	inline storage_linear::size_type storage_linear::image_offset(extent3d const& Coord, extent3d const& ImageExtent) const
	{
		GLI_ASSERT(glm::all(glm::lessThan(Coord, ImageExtent)));
		return static_cast<storage_linear::size_type>(Coord.x + Coord.y * ImageExtent.x + Coord.z * ImageExtent.x * ImageExtent.y);
	}
		...
		extent_type const& CopyBlockCount)
	{
		...
		for(size_t BlockIndexZ = 0, BlockCountZ = CopyBlockCount.z; BlockIndexZ < BlockCountZ; ++BlockIndexZ)
		for(size_t BlockIndexY = 0, BlockCountY = CopyBlockCount.y; BlockIndexY < BlockCountY; ++BlockIndexY)
		{
			...
			memcpy(DataDst, DataSrc, this->block_size() * CopyBlockCount.x);
		}
	}
